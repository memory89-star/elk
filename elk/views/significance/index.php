...
use frontend\modules\audit\models\Step;
...
$this->title = 'Значимость несоответствия';
    ...
    <div class="d-flex justify-content-between toolbar pb-0 pl-2 pr-2">
        <div>
            ...
            if (Yii::$app->user->can('elk_admin')||Yii::$app->user->can('elk_user_gxk')) {
                ...
                'value' => Url::to(['create']),
                'title' => 'Добавить Значимость несоотвествия',
                'class' => 'showModalButton btn btn-shadow btn-success']);
            }
            ?>
            <?= Html::a('Очистить фильтр',  ['clear-filter','modelClassName' => frontend\modules\elk\models\Significance::className()], ['class' => 'btn btn-shadow btn-danger']) ?>
        ...
        <div>
            ...
            <ul class="dropdown-menu exp-btn" x-placement="bottom-start" style="position: absolute; will-change: transform; top: 0px; left: 0px; transform: translate3d(0px, 38px, 0px);">
                <li><a class="dropdown-item" href="/frontend/web/elk/significance/index?per-page=20">20</a></li>
                <li><a class="dropdown-item" href="/frontend/web/elk/significance/index?per-page=50">50</a></li>
                <li><a class="dropdown-item" href="/frontend/web/elk/significance/index?per-page=100">100</a></li>
            </ul>
        </div>
    </div>
