...
$this->title = 'Мероприятия по устранению причин несоотвествия';
...
<div class="kod-index">
    ...
    <h4>
        <?='Несоответствие: '?> <b>  <?= $model->getReestrInd($id) ?>  </b>
    </h4>
    ...
    <h4>
        <?='Причина несоответствия: '?> <b>  <?= $id_discrepancy[$id] ?>  </b>
    </h4>
    ...
    <div class="d-flex justify-content-between toolbar pb-0 pl-2 pr-2">
        <div>
            ...
            echo Html::button('Добавить', [
                ...
                'title' => 'Добавить Мероприятия блока "Корректирующие действия"',
                'class' => 'showModalButton btn btn-shadow btn-success']);
            }
            ?>

        <?= Html::a('Очистить фильтр',  ['clear-filter', 'id' => $id], ['class' => 'btn btn-shadow btn-danger']) ?>
        <?= Html::a('Закрыть', ['/elk/discrepancy', 'id' => $model->getDiscrID($id)], ['class' => 'btn btn-shadow btn-secondary']) ?>
    ...
    <?= GridView::widget([
        ...
        'columns' => [
            ...
            [
                'attribute' => 'id_otvetst',
                'format' => 'raw',
                'value' => function ($data) {
                    $model = new Card();
                    return $model->getFioByCardId($data['id_otvetst']);
                }
            ],

            [
                'attribute' => 'id_kontrol',
                'format' => 'raw',
                'value' => function ($data) {
                    $model = new Card();
                    return $model->getFioByCardId($data['id_kontrol']);
                }
            ],
